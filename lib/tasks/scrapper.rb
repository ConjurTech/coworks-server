...
def getCompany(url)
	...
	logo_url = nameDiv.css('a img').first['src']
	...
	while i < elements.length do
		...
		when "Telephone :"
			data = elements[i=i+1]
			company[:phone_number] = data.css('> text()').text.strip + data['data-last'].strip
		when "Fax :"
			data = elements[i=i+1]
	...
	tags = middleDiv.css('#product div a').map {|a| a['title']}
...
# getCompany('http://www.yellowpages.com.sg/company/menara-freight-consolidators-m-sdn-bhd')
# getCompany('http://www.yellowpages.com.sg/company/v8-environmental-pte-ltd')
# getCompany('http://www.yellowpages.com.sg/company/lj-investigation-consultancy-services-pte-ltd')
# getCompany('http://www.yellowpages.com.sg/company/chun-hoe-trading-sdn-bhd')
